...
<!-- 左侧边栏 -->
<%@include file="/WEB-INF/view/common/messageLeftSidebar.jsp"%>
			...
			<div class="col-md-12">
				<div class="panel">
					<div class="panel-heading">
						<h3 class="panel-title">${language.equals("en_US")?"All Message":"所有消息"}</h3>
						<div class="right">
							<a href="javascript:;" onclick="deleteAllMessage(${user.getID() })">
								<span class="label label-danger"><i class="fa fa-trash"></i>&nbsp;${language.equals("en_US")?"Empty All Messages":"清空全部消息"}</span>
							</a>
						</div>
					</div>
					<div class="panel-body" style="text-align:center;font-size:24px">
						${language.equals("en_US")?"Here are all the messages you've received":"这里是你收到的所有消息"}
					</div>
				</div>
			</div>
				...
				<c:when test="${mes.getReaded()==false }">
					<div class="panel">
						...
						<div class="panel-footer">
							...
							<span style="margin-left:50px;">${language.equals("en_US")?"Identifier":"编号"}:${mes.getIden() }</span>
			...
			<c:if test="${message.size()==0 }">
				<div class="col-md-12">
					<div class="panel">
						...
						<div class="panel-body">
							<p style="text-align:center;font-size:20px">${language.equals("en_US")?"You haven't received any messages yet!!!":"你还没有收到任何消息！！！"}</p>
						</div>
					</div>
				</div>
			</c:if>
