package com.stsm.controller.message;
...
/**
 * Servlet implementation class UnReadMessage
 */
@WebServlet(urlPatterns = {"/message/unread_message","/message"})
public class UnReadMessage extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(id!=0){
			...
			if(count==pageSize&&pageNo!=pagination.getTotalPages()){
				pageNo++;
				...
				response.sendRedirect(url+"page="+pageNo);
				return;
			}
		}
		...
		request.getRequestDispatcher("/WEB-INF/view/message/unreadMessage.jsp").forward(request, response);
	}
